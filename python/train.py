...
def train_and_eval(model):
    for n in range(FLAGS.train_epochs):
        ...
        for f in train_data_list:
            ...
            model.train(
                input_fn=lambda: input_fn(f, 'train', FLAGS.batch_size),
                hooks=None,
                steps=None,
                max_steps=None,
        ...
        if (n+1) % FLAGS.epochs_per_eval == 0:
            ...
            results = model.evaluate(
                input_fn=lambda: input_fn(FLAGS.test_data, 'eval', FLAGS.batch_size),
                 steps=None,  # Number of steps for which to evaluate model.
                 hooks=None,
...
def dynamic_train(model):
    ...
    for i in range(len(data_files)-1):
        ...
        for n in range(FLAGS.train_epochs):
            ...
            t0 = time.time()
            results = model.evaluate(
                input_fn=lambda: input_fn(test_data, 'eval', FLAGS.batch_size),
                steps=None,  # Number of steps for which to evaluate model.
                hooks=None,
                checkpoint_path=None,  # latest checkpoint in model_dir is used.
                name=None)
...
def main(unused_argv):
    ...
    if CONFIG.train['dynamic_train']:
        ...
        print("Using train_and_eval mode.")
