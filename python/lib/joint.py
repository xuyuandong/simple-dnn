...
def _wide_deep_combined_model_fn(
        ...
        config=None):
    """Wide and Deep combined model_fn. (Dnn, Linear)
    Args:
    ...
    input_layer_partitioner = input_layer_partitioner or (
        tf.min_max_variable_partitioner(max_partitions=num_ps_replicas, min_slice_size=64 << 20))
    # weight decay lr
    ...
    if model_type == 'wide' or not dnn_feature_columns:
        ...
        with tf.variable_scope(
            ...
            dnn_logits = dnn_logit_fn(features=features, seq_features = seq_features, mode=mode)
